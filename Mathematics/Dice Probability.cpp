...
long double n, a, b, dp[103][603], res;
...
int main()
{
  ...
  for (int i = 2; i <= n; i++) {
  	for (int j = 1; j <= b; j++) {
  	  for (int k = 1; k <= 6; k++) {if (j-k >= 0) dp[i][j] += dp[i-1][j-k];}
  	  dp[i][j]/=6;
  	}
  }
  
  //for (int i = 1; i <= n; i++) {for (int j = 1; j <= b; j++) {cout << dp[i][j] << " ";} cout << "\n";
  
  for (int i = a; i <= b; i++) res+= dp[(int)n][i];
